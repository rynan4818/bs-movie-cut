...
class Modaldlg_setting < VRModalDialog
  ...
  def construct
    self.caption = '設定'
    self.move(226,71,655,794)
    addControl(GroupBox_Preview,'groupBox_Preview',"プレビュー時のFFmpegオプション",24,406,296,84)
    addControl(VRButton,'button_cancel',"キャンセル",392,690,96,32)
    addControl(VRButton,'button_db_select',"選択",560,40,56,24)
    addControl(VRButton,'button_default',"デフォルト",464,562,88,24)
    addControl(VRButton,'button_hdt_select',"選択",560,96,56,24)
    addControl(VRButton,'button_ok',"OK",520,690,96,32)
    addControl(VRButton,'button_opendir_select',"選択",488,254,56,24)
    addControl(VRButton,'button_parameter',"パラメータ説明",464,586,152,24)
    addControl(VRButton,'button_preview_select',"選択",376,318,56,24)
    addControl(VRButton,'button_preview_temp',"選択",552,374,64,24)
    addControl(VRButton,'button_subtitle_temp',"選択",552,522,64,24)
    addControl(VRCheckbox,'checkBox_ascii',"ASCII印字可能文字以外を削除",24,682,296,24)
    addControl(VRCheckbox,'checkBox_japanese',"日本語モード (再起動後有効)",376,634,240,24)
    addControl(VRCheckbox,'checkBox_newcheck',"最新バージョンをチェックする",24,634,264,24)
    addControl(VRCheckbox,'checkBox_no_message',"タイムスタンプチェックの確認メッセージを表示せず保存した物を優先する。",24,182,600,24)
    addControl(VRCheckbox,'checkBox_stop_time_menu',"終了時間にmenuTimeではなくendTimeを使用する。",24,554,400,24)
    addControl(VRCheckbox,'checkBox_timesave',"元動画ファイルのタイムスタンプをデータベースに保存し読込する。(推奨)",24,150,600,24)
    ...
    addControl(VREdit,'edit_max_volume',"0",520,414,56,24)
    addControl(VREdit,'edit_offset',"0.0",520,454,56,28)
    ...
    addControl(VRStatic,'static10',"メニューのファイル→動画を開くで最初に表示するフォルダ",24,230,448,24)
    addControl(VRStatic,'static11',"標準拡張子",544,230,80,24)
    addControl(VRStatic,'static12',"音声の正規化",344,414,96,24)
    addControl(VRStatic,'static13',"dB",584,414,24,24)
    addControl(VRStatic,'static14',"最大値=",456,414,64,24)
    addControl(VRStatic,'static15',"HMDDistance.dat ファイル",24,72,200,24)
    addControl(VRStatic,'static2',"時間表記",32,586,72,24)
    addControl(VRStatic,'static3',"(注意:インストールパスはASCII文字以外不可)",24,706,336,24)
    addControl(VRStatic,'static4',"プレビューに使用するツール",24,294,232,24)
    addControl(VRStatic,'static5',"プレビュー用一時動画ファイル (SSD劣化防止の為HDDを推奨)",24,350,464,24)
    addControl(VRStatic,'static6',"カットの全体オフセット",344,458,176,24)
    addControl(VRStatic,'static7',"秒",584,458,32,24)
    addControl(VRStatic,'static8',"スコア字幕用一時動画ファイル [パスに半角空白不可] (SSD劣化防止でHDDを推奨)",24,498,600,24)
    addControl(VRStatic,'static9',"ツール用オプション",448,294,168,24)
  end 
