...
class Modaldlg_db_view < VRModalDialog
  ...
  def construct
    ...
    addControl(VRButton,'button_add_folder',"検索対象フォルダを追加する",312,296,240,32)
    addControl(VRButton,'button_cancel',"キャンセル",336,360,96,40)
    addControl(VRButton,'button_ok',"OK",464,360,88,40)
    addControl(VRCheckbox,'checkBox_allread',"全期間",232,16,88,24)
    addControl(VRCheckbox,'checkBox_cut_only',"カット済み動画のみ検索対象とする",32,360,288,24)
    ...
    addControl(VRListbox,'listBox_search_dir',"",32,168,520,96,0x4000)
    addControl(VRStatic,'static1',"開始日時",48,48,88,24)
    addControl(VRStatic,'static10',"動画検索対象フォルダ",40,144,208,24)
    addControl(VRStatic,'static11',"設定の「動画を開くフォルダ」と「出力フォルダ」は自動追加されます",32,272,520,24)
    addControl(VRStatic,'static12',"元動画ファイルはDBにタイムスタンプ保存された物が対象です",24,304,280,40)
...
class Modaldlg_setting < VRModalDialog
  ...
  def construct
    ...
    addControl(VRStatic,'static8',"スコア字幕用一時動画ファイル (SSD劣化防止の為HDDを推奨)",24,408,480,24)
